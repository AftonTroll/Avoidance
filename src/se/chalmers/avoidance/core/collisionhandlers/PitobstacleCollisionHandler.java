...
public class PitobstacleCollisionHandler implements CollisionHandler {
	...
	public void handleCollision(Entity movingEntity, Entity obstacle) {
		...
		if (groupManager.getEntities(GameConstants.PLAYER_GROUP).contains(movingEntity) && 
				groupManager.getEntities(GameConstants.PITOBSTACLES_GROUP).contains(obstacle)) {
		...
		if(groupManager.getEntities(GameConstants.ENEMIES_GROUP).contains(movingEntity) && 
				groupManager.getEntities(GameConstants.PITOBSTACLES_GROUP).contains(obstacle)){
			world.deleteEntity(movingEntity);
			Score score = scoreMapper.get(world.getManager(TagManager.class).getEntity(GameConstants.SCORE_TAG));

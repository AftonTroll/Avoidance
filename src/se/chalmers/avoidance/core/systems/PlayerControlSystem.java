...
/**
 * System that handles the input of the user. and updates the players velocity
 * and position
 * 
 * @author Filip Brynfors
 * 
 */
public class PlayerControlSystem extends EntitySystem implements
	...
	/**
	 * This method is called when the player is to be updated. Updates the
	 * velocity and position of the player
	 * 
	 * @param entities
	...
	protected void processEntities(ImmutableBag<Entity> entities) {
		...
		if (entity != null) {
			...
			float startVelX = Utils.getHorizontalSpeed(playerVel.getSpeed(),
					playerVel.getAngle());
			float startVelY = Utils.getVerticalSpeed(playerVel.getSpeed(),
					playerVel.getAngle());
			...
			// Apply friction
			newSpeed *= Math.pow(frictionMapper.get(entity).getFriction(),
					world.delta);
			...
			playerTransform.setDirection((float) Math.atan2(lastAccelerationY,
					lastAccelerationX));
			float speed = playerVel.getSpeed();
			float angle = playerVel.getAngle();
			float dx = world.delta
					* (startVelX + Utils.getHorizontalSpeed(speed, angle)) / 2;
			float dy = world.delta
					* (startVelY + Utils.getVerticalSpeed(speed, angle)) / 2;
	 ...
	 * 
	 * @param event
	 *            the propertyChangeEvent containing the values of the
	 *            accelerometer
	 */
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (event.getNewValue() != null) {
			...
			if ("touch".equals(event.getPropertyName())
					&& tagManager.getEntity(GameConstants.TAG_PLAYER)
							.getComponent(Jump.class).getJumpCooldownLeft() == 0) {
				jumpMapper.get(tagManager.getEntity(GameConstants.TAG_PLAYER))
						.setInTheAir(true);
			}
		}
	}
